...
var orbiting_planet
var theta : float
var new_linear_velocity = Vector2.ZERO
...
func _physics_process(delta: float) -> void:
	...
	for planet in get_tree().get_nodes_in_group("planets"):
		...
		if distance.length() < 200:
			...
			angle = acos(distance.dot(velocity)/(distance.length()*velocity.length()))
	...
	if distance.length() < ORBIT_HEIGHT: # velocity angle tangental to orbit, cancel excess tangential velocity
		theta += (ORBIT_SPEED * delta)
		position = orbiting_planet.get_position() + ORBIT_HEIGHT*Vector2(cos(theta), sin(theta))
		...
		rotation_degrees = 180+rad_to_deg(theta)
	elif orbiting_planet:
		var rel_rocket = position - orbiting_planet.get_position()
		theta = acos(rel_rocket.dot(Vector2(1,0))/(rel_rocket.length()))
...
#func _draw():
	...
	#draw_line(Vector2.ZERO, velocity, Color(1,0,1), 3)
	...
	if is_launching and event is InputEventScreenTouch:
		if !event.is_pressed():
			if (position-event.position).length() > 25:
				...
				velocity += (position-event.position)*ROCKET_FORCE / MASS
